...
// Add your Ola Maps API key here (from environment variable)
const OLA_API_KEY = import.meta.env.VITE_OLA_API_KEY || "r2F9ycSe4LQVCoTnyBhSlfgLjuHHlwgIpoqsytwU";
...
const OlaMap = ({ onSelectLocation, isLocked = false }: MapProps) => {
  ...
  /* Reverse geocode - Call Ola Maps API directly from frontend */
  const reverseGeocode = async (lat: number, lng: number): Promise<string> => {
    try {
      ...
      // Parse the response - adjust based on actual API response
  ...
  /* Search locations - Call Ola Maps API directly from frontend */
  const handleSearch = async (query: string) => {
    ...
    try {
      const response = await fetch(
        `https://api.olamaps.io/places/v1/geocode?query=${encodeURIComponent(query)}`,
        {
          headers: {
            "X-API-Key": OLA_API_KEY,
          },
      ...
      if (!response.ok) {
        console.error(`Ola Maps search error: ${response.status}`);
        setSearchResults([]);
